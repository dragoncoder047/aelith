...
export function splash(where: Vec2, color: Color | (() => Color), n = 10, yv_max = -200, ig_tags_extra: string[] | true = [], drag: number = 0, wf: number = 1) {
    for (var i = 0; i < n; i++) {
        makeParticle(where, color, ig_tags_extra, drag, wf).applyImpulse(K.vec2(K.rand(-100, 100), K.rand(yv_max, 0)));
    }
}

export function makeParticle(where: Vec2, color: Color | (() => Color), ig_tags_extra: string[] | true = [], drag: number = 0, wf: number = 1) {
    return K.add([
        K.pos(where),
        K.sprite("particle"),
        K.anchor("center"),
        K.scale(K.rand(2, 4)),
        K.layer("particles"),
        ...(typeof ig_tags_extra === "boolean" ? [] : [K.area({
