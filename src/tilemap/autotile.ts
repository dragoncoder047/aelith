    ...
    for (var y = 0; y < tiles.length; y++) {
        ...
        for (var x = 0; x < row.length; x++) {
            ...
            for (var n = 0; n < tilesHere.length; n++) {
                ...
                const check = (dx: number, dy: number) => (tiles[y + dy]?.[x + dx] ?? []).some(t => (tilesWith ?? tile.tags).some(t2 => t.tags.includes(t2)));
                const up = check(0, -1);
                const right = check(1, 0);
                const down = check(0, 1);
                const left = check(-1, 0);
                var pattern: number;
                if (bits === 4) {
                    pattern = (up ? 1 : 0) | (right ? 2 : 0) | (down ? 4 : 0) | (left ? 8 : 0);
                } else if (bits === 8) {
                    const upRight = up && right && check(1, -1);
                    const upLeft = up && left && check(-1, -1);
                    const downRight = down && right && check(1, 1);
                    const downLeft = down && left && check(-1, 1);
                ...
                const chosenFrame = candidateFrames[(candidateFrames.length * hashPoint(tile.pos)) | 0]!;
