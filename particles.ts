...
export function splash(where: Vec2, color: Color | (() => Color), n = 10, yv_max = -200, ig_tags_extra: string[] = []) {
    for (var i = 0; i < n; i++) {
        makeParticle(where, color, ig_tags_extra).applyImpulse(K.vec2(K.rand(-100, 100), K.rand(yv_max, 0)));
    }
}

export function makeParticle(where: Vec2, color: Color | (() => Color), ig_tags_extra: string[] = []) {
    return K.add([
        K.pos(where),
        K.sprite("particle"),
        K.anchor("center"),
